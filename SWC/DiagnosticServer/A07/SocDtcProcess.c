 ...
 *           Config:  D:/code/A07/VIP_A07/Davinci/Gen/HUT_V3_5.dpa
 *        SW-C Type:  SocDtcProcess
 *  Generation Time:  2021-11-01 18:20:58
 *
 ...
 *********************************************************************************************************************/
 ...
 #define DCMEX_DTC_NUMBER_LOG        (300u)   // need >= DCMEX_DTC_NUMBER in DcmEx_Cfg.c
 ...
 *********************************************************************************************************************/
    uint8 Dem_VconfigFuelType = 0u;
    uint8 Dem_VconfigFeatureType = 0u;

    VConfig_GetKindConfig(VCONFIG_KIND_FUEL,&Dem_VconfigFuelType);
    VConfig_GetKindConfig(VCONFIG_KIND_ELECTROMOTOR_POSITION,&Dem_VconfigFeatureType);
    if(((Dem_VconfigFuelType == VCONFIG_FEATURE_DAA40) ||
        (Dem_VconfigFuelType == VCONFIG_FEATURE_DAA50)) &&
       (Dem_VconfigFeatureType == VCONFIG_FEATURE_CBA60))
    {
 ...
 *********************************************************************************************************************/
    ...
    //Rte_Call_rpCS_ComEx_ReceiveSignal(COMEX_SIGNAL_RX_VEHSPDVLD_0X265, &sgAbs3Ptr.VehSpdVld_0x265, sizeof(sgAbs3Ptr.VehSpdVld_0x265));
    //Rte_Call_rpCS_ComEx_ReceiveSignal(COMEX_SIGNAL_RX_VEHSPD_0X265, &sgAbs3Ptr.VehSpd_0x265, sizeof(sgAbs3Ptr.VehSpd_0x265));
    ...
    //Rte_Call_rpCS_ComEx_ReceiveSignal(COMEX_SIGNAL_RX_TMCU_ACTROTSPD, &mcuActSpd, sizeof(mcuActSpd));
 ...
 *********************************************************************************************************************/
    ...
    if(NS_SIGNAL_OK(KL15RetValue) && (KL15Data == 2U))
    {
    ...
    if(((SocVConfigDataBuf[VCONFIG_KIND_FUEL] == VCONFIG_FEATURE_DAA40) ||
        (SocVConfigDataBuf[VCONFIG_KIND_FUEL] == VCONFIG_FEATURE_DAA50)) &&
       (SocVConfigDataBuf[VCONFIG_KIND_ELECTROMOTOR_POSITION] == VCONFIG_FEATURE_CBA60))
    {
        ret = Rte_Call_rpCS_ComEx_ReceiveSignal(COMEX_SIGNAL_RX_TMCU_ACTROTSPD, &mcuActSpd, sizeof(mcuActSpd));
        if((E_OK != ret) && (COMEX_UPDATE_RECEIVED != ret))
        {
            mcuActSpd = 0xFFFF;
        }
    }*/
...
static Std_ReturnType SocDiagnosticMonitorSetEventStatus(uint32 Dtc, Dem_EventStatusType EventStatus)
{
    ...
    static Dem_EventStatusType dtcStatusLast[DCMEX_DTC_NUMBER_LOG] = {0xff};
    ...
    for(i = 0; i < dcmEx_cfg.dtcNumber; i++)
    {
        if(dcmEx_cfg.dtcCfgs[i].dtc == Dtc)
        {
            ...
            if(DCMEX_DTC_NUMBER_LOG > i)
            {
                if(dtcStatusLast[i] != EventStatus)
                {
                    can_warn("Soc report dtc 0x%6X status 0x%x.", dcmEx_cfg.dtcCfgs[i].dtc, EventStatus);
                }
                dtcStatusLast[i] = EventStatus;
            }
            break;
        }
    }
