 ...
 *  -------------------------------------------------------------------------------------------------------------------
 *             File:  Can_Server.c
 *           Config:  D:/code/B16/VIP_B16/Davinci/Gen/HUT_V3_5.dpa
 *        SW-C Type:  Can_Server
 *  Generation Time:  2022-01-25 10:54:51
 *
...
{
    /* V3.5_SRS_SWR19_硬按键_V2.8_20220127.pdf */
...
#define ABNORMAL_NOSLEEP_TIMEOUT 360000  // unit:10ms  60Min
#define ACTIVE_WU_COUNTER_THRESHOLD 20
...
typedef struct
{
    ...
    uint32 unsleep_dtc_counter;
    uint8  wakeup_status;
    uint16  wakeup_counter;
    uint8  activeWU_counter;
    uint16  nvm_counter;
    NA_DtcStatus unsleepDtcStatus;
    NA_DtcStatus unsleepDtcStatusLast;
    NA_DtcStatus wakeupDtcStatus;
    NA_DtcStatus wakeupDtcStatusLast;
    uint16 nmLostCounter;
    uint8  nmLostStatus;
    uint8 vconfigKindFuel;
}NetWork_Abnormal;
 ...
 * Enumeration Types:
 * ==================
 * BswM_BswMRteModeBusStateReq: Enumeration of integer in interval [0...255] with enumerators
 *   BLOCK_STATE (1U)
 *   UNBLOCK_STATE (0U)
 ...
 *   Explicit S/R API:
 *   -----------------
 *   Std_ReturnType Rte_Write_Request_CanServerBusStateReq_requestedMode(BswM_BswMRteModeBusStateReq data)
 *
 * Mode Interfaces:
 * ================
    ...
    canNmRet = CanNm_GetState(0, &nmState, &nmMode);
     if(E_OK == canNmRet)
     {
         if((nmState == NM_STATE_PREPARE_BUS_SLEEP) || (nmState == NM_STATE_BUS_SLEEP) || (nmState == NM_STATE_UNINIT))
         {
            ...
            {
                /* Change block state for BswM Action Rule to next trigger */
                Rte_Write_Request_CanServerBusStateReq_requestedMode(BLOCK_STATE);
            }
         }
         else
         {
...
FUNC(boolean, COM_APPL_CODE) ComIPdu_userData_Callback_tx(PduIdType PduId, P2VAR(PduInfoType, AUTOMATIC, COM_APPL_DATA) PduInfoPtr)
{
    ...
    else
    {
        ...
        {
            /* Unlock app messages to can bus by BswM */
            Rte_Write_Request_CanServerBusStateReq_requestedMode(UNBLOCK_STATE);
        }
        else
        {
            /* do noting */
...
void MD_PduCallback(uint16 pduId, uint8 length)
{
    if (FALSE != md_data.globalStatus)
    {
        ...
        if (pduId < md_cfg.pduNumber)
        {
            ...
            // else if(MD_PDU_NO_ID != md_cfg.pduCfgs[pduId].messageId)
            // {
            //     can_err("MD pdu %d rx len err %d-%d", pduId, length, md_cfg.pduCfgs[pduId].length);
            // }
        }
    }
    //  for NetWorkAbnormal_NmJudge
    if(pduId == MD_PDU_NMRANGE)
    {
        na_data.nmLostCounter = 0;
        na_data.nmLostStatus = FALSE;
    }
}
...
void MD_MainFunction(void)
{
    ...
    if (md_data.globalStatus != globalRet)
    {
        ...
        can_info("Missing DTC switch %s", globalRet==TRUE ? "ON" : "OFF");
    }
...
{
    uint8 i = 0;
    VconfigKind_Allbuffers vConfigData = {0};

    na_data.unsleep_counter = 0u;
    na_data.unsleep_dtc_counter = 0u;
    ...
    na_data.unsleepDtcStatus = NA_DTC_STATUS_IDLE;
    na_data.unsleepDtcStatusLast = NA_DTC_STATUS_IDLE;
    na_data.wakeupDtcStatus = NA_DTC_STATUS_IDLE;
    na_data.wakeupDtcStatusLast = NA_DTC_STATUS_IDLE;

    na_data.nmLostCounter   = 0;
    na_data.nmLostStatus    = 0xFF;
    ...
    Rte_Read_Vconfig_GetKindBuffers_AllKinds(vConfigData);
    na_data.vconfigKindFuel = vConfigData[VCONFIG_KIND_FUEL];
...
/**
 * @brief CanServer_NetWorkAbnormal_HighVoltageJudge
 * 
 * @return E_OK: The high voltage system is powered on; E_NOT_OK: other
 */
static uint8 CanServer_NetWorkAbnormal_HighVoltageJudge(void)
{
    uint8 ret = E_NOT_OK;
    uint8 comExRet = 0;
    uint8 bmsPosrlysts = 0;
    uint8 bmsPowerUpCmplt = 0;

    if((na_data.vconfigKindFuel == VCONFIG_FEATURE_DAA40) || (na_data.vconfigKindFuel == VCONFIG_FEATURE_DAA50)) // PHEV/HEV
    {
        #ifdef COMEX_SIGNAL_RX_BMS_POWERUPCMPLT
        comExRet = Rte_Call_rpCS_ComEx_ReceiveSignal(COMEX_SIGNAL_RX_BMS_POWERUPCMPLT, &bmsPowerUpCmplt, sizeof(bmsPowerUpCmplt));
        if (bmsPowerUpCmplt == 0x01) // 0x1: Complete
        {
            ret = E_OK;
        }
        else // 0x0: Not complete
        {
            ret = E_NOT_OK;
        }
        #endif // COMEX_SIGNAL_RX_BMS_POWERUPCMPLT
    }
    else if(na_data.vconfigKindFuel == VCONFIG_FEATURE_DAA60) // EV
    {
        #ifdef COMEX_SIGNAL_RX_BMS_POSRLYSTS
        comExRet = Rte_Call_rpCS_ComEx_ReceiveSignal(COMEX_SIGNAL_RX_BMS_POSRLYSTS, &bmsPosrlysts, sizeof(bmsPosrlysts));
        if (bmsPosrlysts == 0x01) // 0x1: Close
        {
            ret = E_OK;
        }
        else // 0x0: Open
        ...
        #endif // COMEX_SIGNAL_RX_BMS_POSRLYSTS
    }
    else
    {
        ret = E_NOT_OK;
...
static void CanServer_NetWorkAbnormal_NmJudge(void)
{
    static uint8 nmLostStatusLast = 0xFF;

    if (na_data.nmLostStatus != TRUE)
    {
        na_data.nmLostCounter++;
        if (na_data.nmLostCounter >= (md_cfg.messageCfgs[MD_MESSAGE_NMRANGE].cycle * (md_cfg.messageCfgs[MD_MESSAGE_NMRANGE].missingCycle+2)))
        {
            na_data.nmLostStatus = TRUE;
        }
    }

    if(nmLostStatusLast != na_data.nmLostStatus)
    {
        can_info("NA NM Status: %s", na_data.nmLostStatus == TRUE ? "lost" : "normal");
        nmLostStatusLast = na_data.nmLostStatus;
    }
}

static void CanServer_NetWorkAbnormal_UnsleepDtc(void)
{
    Std_ReturnType canNmRet = E_OK;
    Nm_StateType nmState = NM_STATE_UNINIT;
    Nm_ModeType nmMode = NM_STATE_UNINIT;
    ...
    if(E_OK == canNmRet)
    {
        ...
        {
            na_data.unsleep_dtc_counter = 1;
        }
        else if(nmState == NM_STATE_BUS_SLEEP)
        {
            if((na_data.unsleep_dtc_counter >= 1) && (na_data.unsleep_dtc_counter < ABNORMAL_NOSLEEP_TIMEOUT))
            {
                na_data.unsleep_dtc_counter++;
                if(0 == na_data.unsleep_dtc_counter % 60000) // 10min
                {
                    can_warn("NA unsleep_dtc_counter %d min", na_data.unsleep_dtc_counter/6000);
                }
            }
        }
        else if(nmState == NM_STATE_NORMAL_OPERATION)
        {
            na_data.unsleep_dtc_counter = 0;
        }
    }

    if (na_data.unsleep_dtc_counter >= ABNORMAL_NOSLEEP_TIMEOUT)
    {
        na_data.unsleepDtcStatus = NA_DTC_STATUS_FAIL;
    }
    else
    {
        na_data.unsleepDtcStatus = NA_DTC_STATUS_PASS;
    }
}
...
static void CanServer_NetWorkAbnormal_Unsleep(void)
{
    ...
    CanServer_NetWorkAbnormal_UnsleepDtc();

    if(CanServer_NetWorkAbnormal_HighVoltageJudge() == E_OK)
    {
        return;
    }

    CanServer_NetWorkAbnormal_NmJudge();

    switch(na_data.unsleep_status)
    {
        case NETWORK_ABNORMAL_DETECT:
            if((NS_STATUS_ACTIVE_IGN_ON == ns_data.activeData.statusIgn) && (na_data.nmLostStatus == FALSE))
            {
                na_data.unsleep_counter = 0u;
            }
            else if((NS_STATUS_ACTIVE_IGN_ON != ns_data.activeData.statusIgn) || (na_data.nmLostStatus == TRUE))
            {
                if(na_data.unsleep_counter < ABNORMAL_NOSLEEP_TIMEOUT)
                {
                    na_data.unsleep_counter++;
                    if(0 == na_data.unsleep_counter % 60000) // 10min
                    {
                        can_warn("NA unsleep_counter %d min", na_data.unsleep_counter/6000);
                    }
                    if(na_data.unsleep_counter == ABNORMAL_NOSLEEP_TIMEOUT)
                    {
                        ...
                        else if(na_data.nmLostStatus == FALSE)
                        {
...
static void CanServer_NetWorkAbnormal_Wakeup(void)
{
    ...
    switch(na_data.wakeup_status)
    {
        ...
        case NETWORK_ABNORMAL_WRITENVM:
            ...
            na_data.wakeupDtcStatus = NA_DTC_STATUS_FAIL;
...
void Canserver_NetWorkAbnormal_DtcMonitor(void)
{
    ...
    if ((na_data.unsleepDtcStatusLast != na_data.unsleepDtcStatus) || (na_data.wakeupDtcStatusLast != na_data.wakeupDtcStatus))
    {
        if ((NA_DTC_STATUS_FAIL == na_data.unsleepDtcStatus) || (NA_DTC_STATUS_FAIL == na_data.wakeupDtcStatus))
        {
            retDTC = Rte_Call_DiagnosticMonitor_AbnormalWakeupSleep_0xF01200_SetEventStatus(DEM_EVENT_STATUS_FAILED);
        }
        else if ((NA_DTC_STATUS_PASS == na_data.unsleepDtcStatus) || (NA_DTC_STATUS_PASS == na_data.wakeupDtcStatus))
        {
            retDTC = Rte_Call_DiagnosticMonitor_AbnormalWakeupSleep_0xF01200_SetEventStatus(DEM_EVENT_STATUS_PASSED);
        }
        if(retDTC == E_OK)
        {
            na_data.unsleepDtcStatusLast = na_data.unsleepDtcStatus;
            na_data.wakeupDtcStatusLast = na_data.wakeupDtcStatus;
            can_warn("NA 0xF01200 report %d, %d", na_data.unsleepDtcStatus, na_data.wakeupDtcStatus);
        }
    }
}
