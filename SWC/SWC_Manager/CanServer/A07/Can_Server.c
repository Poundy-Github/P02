 ...
 *  -------------------------------------------------------------------------------------------------------------------
 *             File:  Can_Server.c
 *           Config:  D:/code/A07/VIP_A07_dev/Davinci/Gen/HUT_V3_5.dpa
 *        SW-C Type:  Can_Server
 *  Generation Time:  2022-01-26 10:22:28
 *
...
#define  VEHICLE_TYPE_HEV                               ((uint8)(0u))
#define  VEHICLE_TYPE_FUEL                              ((uint8)(1u))
...
static const Com_Disable_RxPdu_Receive_Info Com_Disable_RxPdu_Receive_Cfg = 
{
    ...
    /*CSA3 BCM6(GW) BCM11 BCM10 BCM8 STBS1*/
    {
        ...
        CanIfConf_CanIfRxPduCfg_STBS1_oB30_for_SC_CAN_V3_2_3fe5270b_Rx
    }, //PduId in Com Layer
...
static FUNC(void, Can_Server_CODE) CanServer_DetermineTxGroup( void )
{
    ...
    if (E_NOT_OK != u8ret)
    {
        if (0x3 == (VConfigInfo[VCONFIG_KIND_FUEL]))
        {
            // A07D  HEV
            Rte_Write_Request_VehicleTypeOptionChoose_requestedMode(VEHICLE_TYPE_HEV);
        }
        else
        {
            // A07  FUEL
...
void Canserver_NetWorkAbnormal_DtcMonitor(void)
{
    ...
    if ((1000 < time++) || (dtcStatusLast != na_data.dtcStatus)) // 10s
    {
        time = 0;
