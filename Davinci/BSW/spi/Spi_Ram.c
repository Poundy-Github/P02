...
/* Hardware statuses */
volatile VAR(Spi_HWStatusType, SPI_VAR_NO_INIT)
...
#endif
volatile VAR(Spi_SeqStatusType, SPI_VAR_NO_INIT)
...
volatile VAR(Spi_ClockModeType, SPI_VAR_NO_INIT) Spi_GenClockModeStatus;
#endif
...
volatile VAR(Spi_ErrorBufferType, SPI_VAR_NO_INIT) Spi_GstErrorBuffer;
#endif

#if (SPI_CSIX_RAM_MIRROR == STD_ON)
  /* Global Array for storing Mirror Data */
volatile VAR(Spi_CSIXCTLRamMirrorType, SPI_VAR_NO_INIT)
  Spi_GaaCTLRegMirror[SPI_MAX_HWUNIT];
#if (SPI_CSIH_CONFIGURED == STD_ON)
volatile VAR(Spi_CSIHRamMirrorType, SPI_VAR_NO_INIT)
  Spi_GaaCSIHRegMirror[SPI_MAX_HWUNIT];
#endif
#if (SPI_CSIG_CONFIGURED == STD_ON)
volatile VAR(Spi_CSIGRamMirrorType, SPI_VAR_NO_INIT)
...
volatile VAR(Spi_DMARamMirrorType, SPI_VAR_NO_INIT)
...
/* END Msg(2:0832)-1 */
  ...
  (SPI_LEVEL_DELIVERED == SPI_LEVEL_2))
/* Bit array indicates active HWUnit index used by sync transmission */
volatile VAR(uint32, SPI_VAR_NO_INIT) Spi_GulActiveSyncHWUnits;
#endif
...
volatile VAR(uint32, SPI_VAR_NO_INIT) Spi_GaaDmaRxData[SPI_MAX_HWUNIT];
#endif
