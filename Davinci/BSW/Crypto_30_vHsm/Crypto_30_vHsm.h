 ...
 * 02.05.01  2020-10-27   vistof  ESCAN00107729 Compiler error: CRYPTO_KEYSETVALID not defined
                                  ...
                                  ESCAN00107730 Undefined behaviour when calling key managmenet APIs
 * 02.06.00 2020-01-05    vistof  HSM-1296      Support NoBuffering driver objects
...
# define CRYPTO_30_VHSM_MAJOR_VERSION                                 (2u)
# define CRYPTO_30_VHSM_MINOR_VERSION                                 (6u)
...
# define CRYPTO_30_VHSM_HOST2HSM_IPC_INITIALIZED                      (0u)
...
/*! Input and output buffers for each driver object */
typedef struct
{
  uint8 *inputPtrBuffer;                    /*!< Pointer to the input buffer */
  uint8 *secondaryInputPtrBuffer;           /*!< Pointer to the secondary input buffer */
  uint8 *tertiaryInputPtrBuffer;            /*!< Pointer to the tertiaryinput buffer */
  uint8 *outputPtrBuffer;                   /*!< Pointer to the output buffer */
  uint32 *outputLengthPtrBuffer;            /*!< Pointer to the output length buffer */
  uint8 *secondaryOutputPtrBuffer;          /*!< Pointer to the secondary output buffer */
  uint32 *secondaryOutputLengthPtrBuffer;   /*!< Pointer to the secondary output length buffer */
  Crypto_VerifyResultType *verifyPtrBuffer; /*!< Pointer to the verifyPtr buffer */
  uint64 *output64PtrBuffer;                /*!< Pointer to the uint64Ptr buffer */
} Crypto_30_vHsm_GlobalDataBufferPtrType;

/*! Driver object array type */
typedef struct
{
  Crypto_30_vHsm_GlobalDataBufferPtrType objects[Crypto_30_vHsm_GetSizeOfObjectInfo()];
} Crypto_30_vHsm_GlobalDataBufferType;

# define CRYPTO_30_VHSM_START_SEC_VAR_INIT_UNSPECIFIED
# include "MemMap.h" /* PRQA S 5087 */ /* MD_MSR_MemMap */

/*! Pointer to global driver object array */
extern P2VAR(Crypto_30_vHsm_GlobalDataBufferType, CRYPTO_30_VHSM_VAR_INIT, CRYPTO_30_VHSM_VAR_NOINIT) Crypto_30_vHsm_DataBufferPtr;
...
/*! Global driver object array */
/* Every driver object has a dedicated place in this array to store its I/O pointers */
extern VAR(Crypto_30_vHsm_GlobalDataBufferType, CRYPTO_30_VHSM_VAR_NOINIT) Crypto_30_vHsm_DataBufferBuffer;
 ...
 *  \param[in]   processingType      Processing type of the job
 ...
 *********************************************************************************************************************/
/*! \brief       Convert the external object id to an internalObjectId
 *  \details     -
 *  \param[in]   objectId            external object Id
 *  \param[out]  internalObjectId    Pointer to the internal object id
 *  \return      E_OK                Internal object Id could be found
 *               E_NOT_OK            External object Id was invalid
 *  \pre         -
 *  \context     TASK
 *  \reentrant   TRUE
 *  \synchronous TRUE
