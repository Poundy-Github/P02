/* Kernbauer Version: 1.16 Konfiguration: DrvCAN Erzeugungsgangnummer: 98 */
...
#endif
#if (DRVCAN_RH850RSCANFDASR_VERSION         != 0x0101u)      
# error "Source and header files are inconsistent!"
#endif
#if (DRVCAN_RH850RSCANFDASR_RELEASE_VERSION != 0x00u)
# error "Source and header files are inconsistent!"
#endif
#if (CAN_GEN_RH850RSCANFDASR_VERSION        != 0x0100u) 
...
#  define kCanEfMaskEccError           0x0001u     /*!< Mask to read ECC Error Flag for all used channels from Global Error Flag Register */
# elif (kCanMaxPhysChannels == 2u)
#  define kCanEfMaskEccError           0x0003u     /*!< Mask to read ECC Error Flag for all used channels from Global Error Flag Register */
# elif (kCanMaxPhysChannels == 3u)
#  define kCanEfMaskEccError           0x0007u     /*!< Mask to read ECC Error Flag for all used channels from Global Error Flag Register */
# elif (kCanMaxPhysChannels == 4u)
#  define kCanEfMaskEccError           0x000Fu     /*!< Mask to read ECC Error Flag for all used channels from Global Error Flag Register */
# elif (kCanMaxPhysChannels == 5u)
#  define kCanEfMaskEccError           0x001Fu     /*!< Mask to read ECC Error Flag for all used channels from Global Error Flag Register */
# elif (kCanMaxPhysChannels == 6u)
#  define kCanEfMaskEccError           0x003Fu     /*!< Mask to read ECC Error Flag for all used channels from Global Error Flag Register */
# elif (kCanMaxPhysChannels == 7u)
#  define kCanEfMaskEccError           0x007Fu     /*!< Mask to read ECC Error Flag for all used channels from Global Error Flag Register */
# elif (kCanMaxPhysChannels == 8u) /* COV_CAN_LL_CONSTANT_DEFINITION_DERIVATIVE_PHYS_CHANNELS */
#  define kCanEfMaskEccError           0x00FFu     /*!< Mask to read ECC Error Flag for all used channels from Global Error Flag Register */
# endif
#endif
...
# endif
# if defined(C_ENABLE_IF_RSCAN_FD_V4) /* COV_CAN_LL_CONSTANT_DEFINITION_DERIVATIVE_IF_RSCAN_FD_V4 */
#  define kCanRamCheckMaskCGCR         0x0000DF00u
#  define kCanRamCheckMaskCRECR        0x0000017Fu
# endif
# if defined(C_ENABLE_IF_RSCAN_FD_V3) /* COV_CAN_LL_CONSTANT_DEFINITION_DERIVATIVE_IF_RSCAN_FD_V3 */
#  if (kCanMaxPhysChannels == 1u)
...
# if defined(C_ENABLE_IF_RSCAN_FD_V4) /* COV_CAN_LL_CONSTANT_DEFINITION_DERIVATIVE_IF_RSCAN_FD_V4 */
#  if (kCanMaxPhysChannels == 1u)
#    define kCanRamCheckMaskCRNCFG0    0x01FF0000u
#  else
#    define kCanRamCheckMaskCRNCFG0    0x01FF01FFu
#  endif
#  if (kCanMaxPhysChannels == 3u)
#    define kCanRamCheckMaskCRNCFG1    0x01FF0000u
#  elif (kCanMaxPhysChannels >= 4u)
#    define kCanRamCheckMaskCRNCFG1    0x01FF01FFu
#  endif
#  if (kCanMaxPhysChannels == 5u)
#    define kCanRamCheckMaskCRNCFG2    0x01FF0000u
#  elif (kCanMaxPhysChannels >= 6u)
#    define kCanRamCheckMaskCRNCFG2    0x01FF01FFu
#  endif
#  if (kCanMaxPhysChannels == 7u)
#    define kCanRamCheckMaskCRNCFG3    0x01FF0000u
#  endif
#  if (kCanMaxPhysChannels == 8u)
#    define kCanRamCheckMaskCRNCFG3    0x01FF01FFu
#  endif
# endif
...
#endif
#if defined(C_ENABLE_IF_RSCAN_FD_V4) /* COV_CAN_LL_CONSTANT_DEFINITION_DERIVATIVE_IF_RSCAN_FD_V4 */
#  define kCanRamCheckMaskCRFCR        0x0001F772u
#endif
...
#endif
#if defined(C_ENABLE_IF_RSCAN_FD_V4) /* COV_CAN_LL_CONSTANT_DEFINITION_DERIVATIVE_IF_RSCAN_FD_V4 */
#  define kCanRamCheckMaskRuleMask     0xFFFFFFFFu
#  define kCanRamCheckMaskRuleBuf      0xFFFFFFFFu
#endif  
# if (kCanMaxPhysChannels == 1u)
...
# endif
# if defined(C_ENABLE_IF_RSCAN_FD_V4) /* COV_CAN_LL_CONSTANT_DEFINITION_DERIVATIVE_IF_RSCAN_FD_V4 */
#  define kCanRamCheckMaskBCFG         0xFFFFFFFFu
#  define kCanRamCheckMaskFDBCFG       0x0F0F1FFFu
#  define kCanRamCheckMaskFDCFG        0x77FF0707u
# endif
...
# if !(CAN_SLEEP_SUPPORT == STD_ON)                                                         || \
     ...
     (!defined(C_ENABLE_USE_OS_INTERRUPT_CONTROL))
...
#define CanLL_ClearBit(num) \
  ((vuint32) ~CanLL_SL32(num))
...
# define CanLL_WordToSharedReg(pch, val) \
  ((((pch)&0x01u) == 0u) ? ((vuint32)(val)) : ((vuint32)((vuint32)(val)<<16)))
...
# define CanLL_FilterListConfig(pch, val) \
  (vuint32)(((vuint32)(val)) << (24u - (((pch) & 0x03u) << 3)))
#endif
#if defined(C_ENABLE_IF_RSCAN_FD_V4) /* COV_CAN_LL_CONSTANT_DEFINITION_DERIVATIVE_IF_RSCAN_FD_V4 */
# define CanLL_FilterListConfig(pch, val) \
  (vuint32)(((vuint32)(val)) << (16u - (((pch) & 0x01u) << 4)))
#endif
...
#if !defined(C_ENABLE_IF_RSCAN_FD_V3 ) && !defined(C_ENABLE_IF_RSCAN_FD_V4)
...
V_DEF_FUNC(CAN_LOCAL_INLINE, vuint8, STATIC_CODE) CanLL_InitBeginIsRegisterCorrupt( CAN_CHANNEL_AND_HW_CHANNEL_CANTYPE_FIRST CanInitParaStructPtr initPara )
{
  ...
  for (idx=0u; idx<3u; idx++)
  {
    ...
    /* TMIECm/TMIECf */ 
    Can->CTBIER[CanLL_TxWordIndex(canPhysChannel)] = CanMemCheckValues32bit[idx];  /* SBSW_CAN_LL02 */
#  if defined (C_ENABLE_IF_RSCAN_FD_V4) /* COV_CAN_LL_CONSTANT_DEFINITION_DERIVATIVE_IF_RSCAN_FD_V4 */
    Can->CTBIER[CanLL_TxWordIndex(canPhysChannel) + 1u] = CanMemCheckValues32bit[idx]; /* SBSW_CAN_LL02 */
#   endif
    ...
    if (   ((Can->ChCtrl[canPhysChannel].BCFG & kCanRamCheckMaskBCFG) != (CanMemCheckValues32bit[idx] & kCanRamCheckMaskBCFG))
        ...
        || ((Can->CTBIER[CanLL_TxWordIndex(canPhysChannel)] != CanMemCheckValues32bit[idx]))
#   if defined (C_ENABLE_IF_RSCAN_FD_V4)  /* COV_CAN_LL_CONSTANT_DEFINITION_DERIVATIVE_IF_RSCAN_FD_V4 */
        || ((Can->CTBIER[CanLL_TxWordIndex(canPhysChannel) + 1u]  != CanMemCheckValues32bit[idx]))
#   endif
  ...
  /* TXQCCm */
  if (   (Can->ChTXQ[0].TQCR[canPhysChannel] != kCanZero)
#   if defined (C_ENABLE_IF_RSCAN_FD_V4) /* COV_CAN_LL_CONSTANT_DEFINITION_DERIVATIVE_IF_RSCAN_FD_V4 */
      || (Can->ChTXQ[1].TQCR[canPhysChannel] != kCanZero)
      || (Can->ChTXQ[2].TQCR[canPhysChannel] != kCanZero)
      || (Can->ChTXQ[3].TQCR[canPhysChannel] != kCanZero)
#   endif
     ) /* COV_CAN_LL_HARDWARE_FAILURE_RAM_CHECK */
  {
    canRegCheckFailed = kCanTrue;
...
# endif
  ...
  CanLL_WriteReg32(CAN_CHANNEL_CANPARA_FIRST &Can->CTBIER[CanLL_TxWordIndex(canPhysChannel)], Can_GetTxIntMask0OfChannelData(canHwChannel), initPara); /* SBSW_CAN_LL02 */
  
#if defined(C_ENABLE_IF_RSCAN_FD_V4) /* COV_CAN_LL_CONSTANT_DEFINITION_DERIVATIVE_IF_RSCAN_FD_V4 */
  CanLL_WriteReg32(CAN_CHANNEL_CANPARA_FIRST &Can->CTBIER[CanLL_TxWordIndex(canPhysChannel)+ 1u], Can_GetTxIntMask1OfChannelData(canHwChannel), initPara); /* SBSW_CAN_LL02 */
#endif
  ...
  Can->CRBRCF[0] = ((vuint32) ~Can_GetRxFcMask0OfChannelData(canHwChannel)); /* SBSW_CAN_LL06 */ /* PRQA S 4399 */ /* MD_Can_IntegerCast */
# if (kCanNumberOfSharedReg16 > 1u)
  Can->CRBRCF[1] = ((vuint32) ~Can_GetRxFcMask1OfChannelData(canHwChannel)); /* SBSW_CAN_LL06 */ /* PRQA S 4399 */ /* MD_Can_IntegerCast */
# endif
# if (kCanNumberOfSharedReg16 > 2u)
  Can->CRBRCF[2] = ((vuint32) ~Can_GetRxFcMask2OfChannelData(canHwChannel)); /* SBSW_CAN_LL06 */ /* PRQA S 4399 */ /* MD_Can_IntegerCast */
# endif
# if (kCanNumberOfSharedReg16 > 3u) /* COV_CAN_LL_CONSTANT_DEFINITION_DERIVATIVE_SHARED_REG */
  Can->CRBRCF[3] = ((vuint32) ~Can_GetRxFcMask3OfChannelData(canHwChannel)); /* SBSW_CAN_LL06 */ /* PRQA S 4399 */ /* MD_Can_IntegerCast */
# endif
#endif
...
# endif
  {
    ...
    Can->CRFCR[initPara->hwObjHandle] &= ((vuint32) ~kCanCrFifoEnable); /* SBSW_CAN_LL03 */ /* PRQA S 4399 */ /* MD_Can_IntegerCast */
    /* #20 delete receive FIFO buffer interrupt and overrun flags */
    Can->CRFSR[initPara->hwObjHandle] &= ((vuint32) ~(kCanSrRxFifoIntFlag | kCanSrFifoOverrun)); /* SBSW_CAN_LL03 */ /* PRQA S 4399 */ /* MD_Can_IntegerCast */
...
#if defined(C_ENABLE_CAN_RAM_CHECK_EXTENDED)
    /* #30 perform the global register RAM pattern check */
    canIsCellCorrupt = CanLL_IsGlobalRegisterCorrupt();
#endif /* C_ENABLE_CAN_RAM_CHECK_EXTENDED */


    /* #40 set global registers */
    CanLL_WriteRegGlobal32(&Can->CGCFG, kCanGlobCfg); /* SBSW_CAN_LL01 */
    CanLL_WriteRegGlobal32(&Can->CGFDCFG, kCanZero); /* SBSW_CAN_LL01 */
#if defined(C_ENABLE_IF_RSCAN_FD_V4) /* COV_CAN_LL_CONSTANT_DEFINITION_DERIVATIVE_IF_RSCAN_FD_V4 */
    CanLL_WriteRegGlobal32(&Can->GFCMC, kCanZero); /* SBSW_CAN_LL01 */
    CanLL_WriteRegGlobal32(&Can->GFTBAC, kCanZero); /* SBSW_CAN_LL01 */
#endif
    /* #50 configure filter rules, receive buffers, receive and transmit FIFO buffers */
...
# endif
    ...
    /* #60 set global control register and transit to global operation mode */
...
#if defined(C_ENABLE_CAN_RAM_CHECK_EXTENDED)
    /* #70 notify corrupt global register RAM */
...
#  endif
  ...
  if (   (Can->CTBTCSR[CanLL_TxWordIndex(canPhysChannel)] != 0u)
#  if defined(C_ENABLE_CANCEL_IN_HW)
      || (Can->CTBASR[CanLL_TxWordIndex(canPhysChannel)] != 0u)
#  endif
#  if defined(C_ENABLE_IF_RSCAN_FD_V4) /* COV_CAN_LL_CONSTANT_DEFINITION_DERIVATIVE_IF_RSCAN_FD_V4 */
      || (Can->CTBTCSR[CanLL_TxWordIndex(canPhysChannel) + 1u] != 0u)
#   if defined(C_ENABLE_CANCEL_IN_HW)
      || (Can->CTBASR[CanLL_TxWordIndex(canPhysChannel) + 1u] != 0u) 
#   endif
#  endif
...
# endif
  {
    ...
    Can->CRFSR[rxBasicPara->hwObjHandle] &= ((vuint32) ~kCanSrRxFifoIntFlag); /* SBSW_CAN_LL03 */ /* PRQA S 4399 */ /* MD_Can_IntegerCast */
...
# endif
  {
    ...
    Can->CRBRCF[CanLL_BitIndex(rxFullPara->hwObjHandle)] = CanLL_ClearBit(rxFullPara->hwObjHandle); /* SBSW_CAN_LL06 */
...
# endif
    ...
    */
    ...
    if ((Can->CRBRCF[CanLL_BitIndex(rxFullPara->hwObjHandle)] & CanLL_SL32(rxFullPara->hwObjHandle)) != 0u) /* COV_CAN_LL_NOT_IN_ALL_CONFIG XF */
    {
...
# if defined(C_ENABLE_HW_LOOP_TIMER)
      ...
      } while ((canHwLoopRetVal != CAN_NOT_OK) && ((Can->CRBRCF[CanLL_BitIndex(rxFullPara->hwObjHandle)] & CanLL_SL32(rxFullPara->hwObjHandle)) != 0u)); /* COV_CAN_LL_NOT_IN_ALL_CONFIG XF tx xf */
# else
      } while ((Can->CRBRCF[CanLL_BitIndex(rxFullPara->hwObjHandle)] & CanLL_SL32(rxFullPara->hwObjHandle)) != 0u); /* COV_CAN_LL_NOT_IN_ALL_CONFIG XF */
# endif
...
V_DEF_FUNC(CAN_LOCAL_INLINE, vuint8, STATIC_CODE) CanLL_RxFullIsGlobalIndPending(CAN_CHANNEL_AND_HW_CHANNEL_CANTYPE_ONLY)
{
  ...
  if (   ((Can->CRBRCF[0] & Can_GetRxFcMask0OfChannelData(canHwChannel)) != 0u)
#if (kCanNumberOfSharedReg16 > 1u)
      || ((Can->CRBRCF[1] & Can_GetRxFcMask1OfChannelData(canHwChannel)) != 0u)
#endif
#if (kCanNumberOfSharedReg16 > 2u)
      || ((Can->CRBRCF[2] & Can_GetRxFcMask2OfChannelData(canHwChannel)) != 0u)
#endif
#if (kCanNumberOfSharedReg16 > 3u) /* COV_CAN_LL_CONSTANT_DEFINITION_DERIVATIVE_SHARED_REG */
      || ((Can->CRBRCF[3] & Can_GetRxFcMask3OfChannelData(canHwChannel)) != 0u)
#endif
     ) /* COV_CAN_LL_NOT_IN_ALL_CONFIG */
  {
    retVal = kCanTrue;
...
V_DEF_FUNC(CAN_LOCAL_INLINE, void, STATIC_CODE) CanLL_RxFullProcessPendings(CAN_CHANNEL_AND_HW_CHANNEL_CANTYPE_FIRST CanTaskParaStructPtr taskPara)
{
  ...
  if ((Can->CRBRCF[CanLL_BitIndex(taskPara->hwObjHandle)] & CanLL_SL32(taskPara->hwObjHandle)) != 0u)
  {
...
# endif
      {
        Can->CRFSR[hwObjHandle] &= ((vuint32) ~kCanSrFifoOverrun); /* SBSW_CAN_LL03 */ /* PRQA S 4399 */ /* MD_Can_IntegerCast */
      }
    }
    retVal = kCanTrue;
  }
...
V_DEF_FUNC(CAN_STATIC, vuint8, STATIC_CODE) CanLL_ModeTransition(CAN_CHANNEL_AND_HW_CHANNEL_CANTYPE_FIRST vuint8 mode, vuint8 busOffRecovery, vuint8 ramCheck)
{
  ...
  else
  {
    ...
    {
      /* #30 transit to channel reset mode */
...
V_DEF_FUNC(CAN_LOCAL_INLINE, void, STATIC_CODE) CanLL_InitPowerOnModeTransitions(void)
{
  ...
  /* #10 wait till CAN RAM initialization is complete */
  if ((Can->CGSR & kCanSrMaskRamIst) != 0u) /* COV_CAN_HARDWARE_FAILURE XF */
  {
    CanLL_CanTimerStart(kCanLoopRamInit); /* SBSW_CAN_LL12 */
#if defined(C_ENABLE_HW_LOOP_TIMER)
    while (((Can->CGSR & kCanSrMaskRamIst) != 0u) && (CanLL_CanTimerLoop(kCanLoopRamInit) != CAN_NOT_OK)) /* PRQA S 3415 */ /* MD_Can_RuntimeOptimization_LL */ /* COV_CAN_HARDWARE_FAILURE */
#else
    ...
    CanLL_CanTimerEnd(kCanLoopRamInit); /* SBSW_CAN_LL12 */
  }

  /* #20 transit to global stop mode in order to stop all channels that may be active */
  if (!CanLL_GlobalModeCheck(kCanStopMode))
  {
    CanLL_GlobalModeReq(kCanResetMode); /* SBSW_CAN_LL01 */
    if (!CanLL_GlobalModeCheck(kCanResetMode)) /* COV_CAN_LL_MODE_CHECK */
    {
      CanLL_GlobalModeWait(kCanResetMode, kCanLoopGlobalInit);
    }
    CanLL_GlobalModeReq(kCanStopMode); /* SBSW_CAN_LL01 */
    if (!CanLL_GlobalModeCheck(kCanStopMode)) /* COV_CAN_LL_MODE_CHECK */
    {
  ...
  /* #30 invoke callout for ECC configuration */
  ApplCanEccConfiguration();
#endif

  /* #40 transit to global reset mode */
  CanLL_GlobalModeReq(kCanResetMode); /* SBSW_CAN_LL01 */
  if (!CanLL_GlobalModeCheck(kCanResetMode)) /* COV_CAN_LL_MODE_CHECK */
  {
    CanLL_GlobalModeWait(kCanResetMode, kCanLoopGlobalInit);
...
V_DEF_FUNC(CAN_LOCAL_INLINE, void, STATIC_CODE) CanLL_InitPowerOnGlobalInit(void)
{
  ...
  CanLL_WriteRegGlobal32(&Can->CRNCFG[0], kCanZero); /* SBSW_CAN_LL02 */
#if defined(C_ENABLE_IF_RSCAN_FD_V3) /* COV_CAN_LL_CONSTANT_DEFINITION_DERIVATIVE_IF_RSCAN_FD_V3 */     
# if (kCanMaxPhysChannels > 4)
  CanLL_WriteRegGlobal32(&Can->CRNCFG[1], kCanZero); /* SBSW_CAN_LL02 */
# endif
#endif
#if defined(C_ENABLE_IF_RSCAN_FD_V4) /* COV_CAN_LL_CONSTANT_DEFINITION_DERIVATIVE_IF_RSCAN_FD_V4 */
# if (kCanMaxPhysChannels > 2)
  CanLL_WriteRegGlobal32(&Can->CRNCFG[1], kCanZero); /* SBSW_CAN_LL02 */
# endif
# if (kCanMaxPhysChannels > 4)
  CanLL_WriteRegGlobal32(&Can->CRNCFG[2], kCanZero); /* SBSW_CAN_LL02 */
# endif
# if (kCanMaxPhysChannels > 6)
  CanLL_WriteRegGlobal32(&Can->CRNCFG[3], kCanZero); /* SBSW_CAN_LL02 */
# endif
#endif
  ...
  {
    CanLL_WriteRegGlobal32(&Can->CRFCR[canFifoIndex], kCanZero); /* SBSW_CAN_LL03 */
  }
...
#if defined(C_ENABLE_IF_RSCAN_FD_V4) /* COV_CAN_LL_CONSTANT_DEFINITION_DERIVATIVE_IF_RSCAN_FD_V4 */
      CanLL_WriteRegGlobal32(&Can->ChFCE[canPhysChannelIndex].CFCCE[canFifoIndex], kCanZero); /* SBSW_CAN_LL05 */
#endif
    }
  }
  ...
  for (channel = 0; channel < kCanNumberOfChannels; channel++) /* CM_CAN_LL04 */
#endif
  {
...
#endif
    {
      ...
      CanLL_WriteRegGlobal32(&Can->ChTXQ[0].TQCR[canPhysChannel], kCanZero); /* SBSW_CAN_LL19 */
      /* tx buffer linkage by transmit / receive FIFOs has already been cleared */
#if defined(C_ENABLE_IF_RSCAN_FD_V4) /* COV_CAN_LL_CONSTANT_DEFINITION_DERIVATIVE_IF_RSCAN_FD_V4 */
      CanLL_WriteRegGlobal32(&Can->ChTXQ[1].TQCR[canPhysChannel], kCanZero); /* SBSW_CAN_LL19 */
      CanLL_WriteRegGlobal32(&Can->ChTXQ[2].TQCR[canPhysChannel], kCanZero); /* SBSW_CAN_LL19 */
      CanLL_WriteRegGlobal32(&Can->ChTXQ[3].TQCR[canPhysChannel], kCanZero); /* SBSW_CAN_LL19 */
#endif
#if defined(C_ENABLE_TX_HW_FIFO)
...
#if defined(C_ENABLE_CAN_FD_FULL)
  CanLL_WriteRegGlobal32(&Can->CRBNUM, ((((vuint32)Can_GetRxBufferPayloadStorageSize()) << 8) | canFcObjTmp)); /* SBSW_CAN_LL01 */
#else
  CanLL_WriteRegGlobal32(&Can->CRBNUM, canFcObjTmp); /* SBSW_CAN_LL01 */
#endif
  ...
  CanLL_WriteRegGlobal32(&Can->CRECR, kCanRuleReceive); /* SBSW_CAN_LL01 */
}
/* CODE CATEGORY 4 END */
...
# endif
  ...
  canTmpReg = (Can->CRNCFG[CanLL_ByteIndex(canPhysChannel)] | CanLL_FilterListConfig(canPhysChannel, Can_GetFrTotalCountOfChannelData(channel)));
  CanLL_WriteRegGlobal32(&Can->CRNCFG[CanLL_ByteIndex(canPhysChannel)], canTmpReg); /* SBSW_CAN_LL02 */

  canFilterRulesIndex = (vuint8) (Can_GetFrStartIndexOfChannelData(channel) & kCanRuleIndexMask);
  canFilterRulesPage = (vuint8) (((vuint16)Can_GetFrStartIndexOfChannelData(channel) & kCanRulePageMask) >> kCanRulePageShiftVal);
  CanLL_WriteRegGlobal32(&Can->CRECR, (vuint32)(kCanRuleWrite | canFilterRulesPage)); /* SBSW_CAN_LL01 */ /* PRQA S 4391 */ /* MD_Can_IntegerCast */
#endif
...
# if defined(C_ENABLE_EXTENDED_ID)
#  if defined(C_ENABLE_MIXED_ID)
    if (((tCanIdType)Can_GetMailboxIDValue(mailboxHandle) & CAN_ID_IDE_MASK) != CAN_ID_IDE_MASK) /* COV_CAN_GENDATA_NOT_IN_ALL_CONFIG */ /* PRQA S 2991, 2995 */ /* MD_Can_ConstValue */
    {
      CanLL_WriteRegGlobal32(&Can->Rule[canFilterRulesIndex].Code, MK_STDID0(Can_GetMailboxIDValue(mailboxHandle))); /* SBSW_CAN_LL07 */ /* PRQA S 4491, 2985 */ /* MD_Can_IntegerCast, MD_Can_ConstValue */
      CanLL_WriteRegGlobal32(&Can->Rule[canFilterRulesIndex].Mask, kCanRuleStdMustMatch); /* SBSW_CAN_LL07 */
    }
    else
#  endif
    {
      CanLL_WriteRegGlobal32(&Can->Rule[canFilterRulesIndex].Code, MK_EXTID0(Can_GetMailboxIDValue(mailboxHandle))); /* SBSW_CAN_LL07 */ /* PRQA S 2880 */ /* MD_Can_ConstValue */
      CanLL_WriteRegGlobal32(&Can->Rule[canFilterRulesIndex].Mask, kCanRuleExtMustMatch); /* SBSW_CAN_LL07 */
    }
# else
    {
      CanLL_WriteRegGlobal32(&Can->Rule[canFilterRulesIndex].Code, MK_STDID0(Can_GetMailboxIDValue(mailboxHandle))); /* SBSW_CAN_LL07 */ /* PRQA S 4491, 2985 */ /* MD_Can_IntegerCast, MD_Can_ConstValue */
...
# endif
    ...
    CanLL_WriteRegGlobal32(&Can->Rule[canFilterRulesIndex].Buf, (kCanRuleUseBuf | (((vuint32)(hwObjHandle)) << 8))); /* SBSW_CAN_LL07 */
    CanLL_WriteRegGlobal32(&Can->Rule[canFilterRulesIndex].Fifo, kCanZero); /* SBSW_CAN_LL07 */
    canFilterRulesIndex++;
  }
#endif /* C_ENABLE_RX_FULLCAN_OBJECTS */
...
# endif
    {
      ...
      {
        CanLL_InitPowerOnRulePageUpdate(&canFilterRulesIndex, &canFilterRulesPage); /* SBSW_CAN_LL16 */ /* CM_CAN_LL09 */
        CanLL_WriteRegGlobal32(&Can->Rule[canFilterRulesIndex].Code, ((vuint32) Can_GetCodeOfInitFilterRules(canFilterRulesBcIndex))); /* SBSW_CAN_LL07 */
        CanLL_WriteRegGlobal32(&Can->Rule[canFilterRulesIndex].Mask, ((vuint32) (kCanRuleHeader | Can_GetMaskOfInitFilterRules(canFilterRulesBcIndex)))); /* SBSW_CAN_LL07 */
        /* store messages in global rx fifos */
        CanLL_WriteRegGlobal32(&Can->Rule[canFilterRulesIndex].Buf, kCanZero); /* SBSW_CAN_LL07 */
        CanLL_WriteRegGlobal32(&Can->Rule[canFilterRulesIndex].Fifo, CanLL_SL32(hwObjHandle)); /* SBSW_CAN_LL07 */ /* PRQA S 2985 */ /* MD_Can_ConstValue */
        canFilterRulesIndex++;
      }

      /* #60 configure receive FIFO objects */
# if defined(C_ENABLE_CAN_FD_FULL)
      canTmpReg = (kCanCrRxFifoSet | ((vuint32)((vuint32)Can_GetFifoDepthOfRxFifoData(canLogHwFifoHandle) << 8)) | ((vuint32)((vuint32)Can_GetFifoPayloadStorageSizeOfRxFifoData(canLogHwFifoHandle) << 4)));
# else
      canTmpReg = (kCanCrRxFifoSet | ((vuint32)((vuint32)Can_GetFifoDepthOfRxFifoData(canLogHwFifoHandle) << 8)));
# endif
...
#  endif
      {
        canTmpReg |= kCanCrRxFifoIntEnable;
      }
# endif
      CanLL_WriteRegGlobal32(&Can->CRFCR[hwObjHandle], canTmpReg); /* SBSW_CAN_LL03 */
    }
# if defined(C_ENABLE_MULTIPLE_BASICCAN)
    hwObjHandle++;
# endif
...
# else
  CanLL_WriteRegGlobal32(&Can->CRECR, kCanRuleWrite); /* SBSW_CAN_LL01 */
...
V_DEF_FUNC(CAN_LOCAL_INLINE, void, STATIC_CODE) CanLL_InitPowerOnRulePageUpdate(CanVarPtr8 canFilterRulesIndexPtr, CanVarPtr8 canFilterRulesPagePtr)
{
  ...
  if (*canFilterRulesIndexPtr >= kCanMaxRuleAccessCell) /* COV_CAN_GENDATA_NOT_IN_ALL_CONFIG */ /* CM_CAN_LL09 */
  {
    ...
    CanLL_WriteRegGlobal32(&Can->CRECR, (vuint32)(kCanRuleWrite | (*canFilterRulesPagePtr))); /* SBSW_CAN_LL01 */ /* PRQA S 4391 */ /* MD_Can_IntegerCast */
...
V_DEF_FUNC(CAN_LOCAL_INLINE, vuint8, STATIC_CODE) CanLL_IsGlobalRegisterCorrupt(void)
{
  ...
  for (idx=0u; idx<3u; idx++)
  {
    ...
    /* CRECR/GAFLECTR */
    Can->CRECR = (CanMemCheckValues32bit[idx] & kCanRamCheckMaskCRECR); /* SBSW_CAN_LL01 */
    /* CRNCFG?/GAFLCFG? */
    Can->CRNCFG[0] = (CanMemCheckValues32bit[idx] & kCanRamCheckMaskCRNCFG0); /* SBSW_CAN_LL02 */
# if defined(C_ENABLE_IF_RSCAN_FD_V3) /* COV_CAN_LL_CONSTANT_DEFINITION_DERIVATIVE_IF_RSCAN_FD_V3 */
#  if (kCanMaxPhysChannels > 4)
    Can->CRNCFG[1] = (CanMemCheckValues32bit[idx] & kCanRamCheckMaskCRNCFG1); /* SBSW_CAN_LL02 */
#  endif
# endif
# if defined(C_ENABLE_IF_RSCAN_FD_V4) /* COV_CAN_LL_CONSTANT_DEFINITION_DERIVATIVE_IF_RSCAN_FD_V4 */
#  if (kCanMaxPhysChannels > 2)
...
#  endif
#  if (kCanMaxPhysChannels > 4)
    Can->CRNCFG[2] = (CanMemCheckValues32bit[idx] & kCanRamCheckMaskCRNCFG2); /* SBSW_CAN_LL02 */
#  endif
#  if (kCanMaxPhysChannels > 6)
    Can->CRNCFG[3] = (CanMemCheckValues32bit[idx] & kCanRamCheckMaskCRNCFG3); /* SBSW_CAN_LL02 */
#  endif
# endif
    /* CRBNUM/RMNB */
    Can->CRBNUM = (CanMemCheckValues32bit[idx] & kCanRamCheckMaskCRBNUM); /* SBSW_CAN_LL01 */
    ...
    if (   ((Can->CGCFG & kCanRamCheckMaskCGCFG) != (CanMemCheckValues32bit[idx] & kCanRamCheckMaskCGCFG))
        ...
        || ((Can->CRECR & kCanRamCheckMaskCRECR) != (CanMemCheckValues32bit[idx] & kCanRamCheckMaskCRECR))
        || ((Can->CRNCFG[0] & kCanRamCheckMaskCRNCFG0) != (CanMemCheckValues32bit[idx] & kCanRamCheckMaskCRNCFG0))
# if defined(C_ENABLE_IF_RSCAN_FD_V3) /* COV_CAN_LL_CONSTANT_DEFINITION_DERIVATIVE_IF_RSCAN_FD_V3 */
#   if (kCanMaxPhysChannels > 4u)  
        || ((Can->CRNCFG[1] & kCanRamCheckMaskCRNCFG1) != (CanMemCheckValues32bit[idx] & kCanRamCheckMaskCRNCFG1))
#   endif
# endif
# if defined(C_ENABLE_IF_RSCAN_FD_V4) /* COV_CAN_LL_CONSTANT_DEFINITION_DERIVATIVE_IF_RSCAN_FD_V4 */
#  if (kCanMaxPhysChannels > 2u) 
        || ((Can->CRNCFG[1] & kCanRamCheckMaskCRNCFG1) != (CanMemCheckValues32bit[idx] & kCanRamCheckMaskCRNCFG1))
#  endif
#  if (kCanMaxPhysChannels > 4u)
        || ((Can->CRNCFG[2] & kCanRamCheckMaskCRNCFG2) != (CanMemCheckValues32bit[idx] & kCanRamCheckMaskCRNCFG2))
#  endif
#  if (kCanMaxPhysChannels > 6u)
        || ((Can->CRNCFG[3] & kCanRamCheckMaskCRNCFG3) != (CanMemCheckValues32bit[idx] & kCanRamCheckMaskCRNCFG3))
#  endif
# endif
        || ((Can->CRBNUM & kCanRamCheckMaskCRBNUM) != (CanMemCheckValues32bit[idx] & kCanRamCheckMaskCRBNUM))
       ) /* COV_CAN_LL_HARDWARE_FAILURE_RAM_CHECK */
    {
    ...
    for (canFifoIndex=0; canFifoIndex<kCanMaxRxFifos; canFifoIndex++) /* CM_CAN_LL05 */
    {
      ...
      Can->CRFCR[canFifoIndex] = (CanMemCheckValues32bit[idx] & kCanRamCheckMaskCRFCR); /* SBSW_CAN_LL03 */
      if ((Can->CRFCR[canFifoIndex] & kCanRamCheckMaskCRFCR) != (CanMemCheckValues32bit[idx] & kCanRamCheckMaskCRFCR)) /* COV_CAN_LL_HARDWARE_FAILURE_RAM_CHECK */
      {
        canGlobalRegCheckFailed = kCanTrue;
      }
    }
    ...
    for (canFilterRulesIndex=0; canFilterRulesIndex<kCanMaxRuleAccessCell; canFilterRulesIndex++) /* CM_CAN_LL09 */
    {
      ...
      Can->Rule[canFilterRulesIndex].Code = (CanMemCheckValues32bit[idx] & kCanRamCheckMaskRuleCode); /* SBSW_CAN_LL07 */
      Can->Rule[canFilterRulesIndex].Mask = (CanMemCheckValues32bit[idx] & kCanRamCheckMaskRuleMask); /* SBSW_CAN_LL07 */
      Can->Rule[canFilterRulesIndex].Buf = (CanMemCheckValues32bit[idx] & kCanRamCheckMaskRuleBuf); /* SBSW_CAN_LL07 */
      Can->Rule[canFilterRulesIndex].Fifo = (CanMemCheckValues32bit[idx] & kCanRamCheckMaskRuleFifo); /* SBSW_CAN_LL07 */

      if (   ((Can->Rule[canFilterRulesIndex].Code & kCanRamCheckMaskRuleCode) != (CanMemCheckValues32bit[idx] & kCanRamCheckMaskRuleCode))
          || ((Can->Rule[canFilterRulesIndex].Mask & kCanRamCheckMaskRuleMask) != (CanMemCheckValues32bit[idx] & kCanRamCheckMaskRuleMask))
          || ((Can->Rule[canFilterRulesIndex].Buf & kCanRamCheckMaskRuleBuf) != (CanMemCheckValues32bit[idx] & kCanRamCheckMaskRuleBuf))
          || ((Can->Rule[canFilterRulesIndex].Fifo & kCanRamCheckMaskRuleFifo) != (CanMemCheckValues32bit[idx] & kCanRamCheckMaskRuleFifo))
         ) /* COV_CAN_LL_HARDWARE_FAILURE_RAM_CHECK */
      {
        canGlobalRegCheckFailed = kCanTrue;
      }
...
#  endif
  if ( 
#  if defined(C_ENABLE_TX_FULLCAN_OBJECTS)
       ((txConfPara.mailboxHandle < Can_GetTxFullHandleStart(canHwChannel)) || (txConfPara.mailboxHandle >= Can_GetTxFullHandleStop(canHwChannel))) &&  /* COV_CAN_GENDATA_FAILURE */
#  endif
...
# endif
  ...
  if (  ( Can_GetBaseDll_GeneratorVersion()     != (uint16)CAN_GEN_BASE_CFG5_VERSION ) ||  /* PRQA S 2742 */ /* MD_Can_ConstValue */
        ( Can_GetPlatformDll_GeneratorVersion() != (uint16)CAN_GEN_RH850RSCANFDASR_VERSION ) ) /* COV_CAN_GENDATA_FAILURE */
  {
...
V_DEF_FUNC_API(V_NONE, vuint8, CODE) CanEccTxError(CanChannelHandle Controller)
{
  ...
  {
    bitmask = CanLL_SL16(mCanLogToPhys(Controller));
  }

  /* #20 return kCanTrue if relevant bits are set */
  if ((Can->CGEFECC & bitmask) != kCanZero) /* COV_CAN_HARDWARE_FAILURE XF */
  {
    retVal = kCanTrue;
  }
...
V_DEF_FUNC_API(V_NONE, void, CODE) CanEccTxErrorClear(CanChannelHandle Controller)
{
  ...
  {
    bitmask = (((vuint16) ~CanLL_SL16(mCanLogToPhys(Controller))) & kCanEfMaskEccError);
  }

  /* #20 clear relevant bits in register */
  Can->CGEFECC = bitmask; /* SBSW_CAN_LL01 */
}
/* CODE CATEGORY 4 END */
#endif
 ...
 */
/* CODE CATEGORY 1 START */
V_DEF_FUNC(V_NONE, void, CODE) CanInterruptRxFifo(void) /* COV_CAN_HWOBJINDIVPOLLING */
{
  /* ----- Local Variables ---------------------------------------------- */
  ...
  CanLL_EI(); /* SBSW_CAN_LL18 */ /* PRQA S 1006 */ /* MD_Can_Assembler_LL */
#     if defined(C_MULTIPLE_RECEIVE_CHANNEL)
    /* #10 iterate channels */
...
#     endif
  ...
  CanLL_DI(); /* SBSW_CAN_LL18 */ /* PRQA S 1006 */ /* MD_Can_Assembler_LL */
...
#  endif 
  vuint8 regCount=0u;
  vuint8 bitIndex;
  vuint32 localMask = Can_GetTxIntMask0OfChannelData(canHwChannel); 
...
#  if defined(C_ENABLE_IF_RSCAN_FD_V4) /* COV_CAN_LL_CONSTANT_DEFINITION_DERIVATIVE_IF_RSCAN_FD_V4 */
  for (; regCount < 2u; regCount++) 
#  endif
  {
#  if defined(C_ENABLE_IF_RSCAN_FD_V4) /* COV_CAN_LL_CONSTANT_DEFINITION_DERIVATIVE_IF_RSCAN_FD_V4 */
    if (regCount != 0u)
    {
      localMask = Can_GetTxIntMask1OfChannelData(canHwChannel);
    }
#  endif
    localReg = Can->CTBTCSR[CanLL_TxWordIndex(canPhysChannel) + regCount] & localMask; /* PRQA S 2985, 2986 */ /* MD_Can_ConstValue */
#  if defined(C_ENABLE_CANCEL_IN_HW)
    /* #20 get transmit abort flags for transmit buffers configured for interrupt handling */
#   if defined(C_ENABLE_INDIVIDUAL_POLLING)
    localReg |= Can->CTBASR[CanLL_TxWordIndex(canPhysChannel) + regCount]; /* PRQA S 2985, 2986 */ /* MD_Can_ConstValue */
#   else
    localReg |= Can->CTBASR[CanLL_TxWordIndex(canPhysChannel) + regCount] & localMask; /* PRQA S 2985, 2986 */ /* MD_Can_ConstValue */
#   endif
#  endif
    ...
    if (localReg != 0u) /* COV_CAN_GENDATA_TX_NOT_IN_ALL_CONFIG */
    {
      ...
      if      ((localReg & 0x000000FFu) != 0u) { bitIndex = 0u; } /* COV_CAN_GENDATA_TX_NOT_IN_ALL_CONFIG */
      else if ((localReg & 0x0000FF00u) != 0u) { bitIndex = 8u; } /* COV_CAN_GENDATA_TX_NOT_IN_ALL_CONFIG */
      else if ((localReg & 0x00FF0000u) != 0u) { bitIndex = 16u; } /* COV_CAN_GENDATA_TX_NOT_IN_ALL_CONFIG */
      else                                     { bitIndex = 24u; }
      ...
      do
      {
        if ((localReg & CanLL_SL32(bitIndex)) != 0u) /* COV_CAN_GENDATA_TX_NOT_IN_ALL_CONFIG */
        {
          hwObjHandle = bitIndex + (regCount * 32u); /* PRQA S 2985 */ /* MD_Can_ConstValue */
...
#  endif
        ...
        bitIndex++;
      }  while (localReg != 0u); /* COV_CAN_GENDATA_TX_NOT_IN_ALL_CONFIG */
    }
  }
#  if defined(C_ENABLE_TX_HW_FIFO)
...
#   endif
    {
      if ((Can->ChFS[canPhysChannel].TRFSR[hwObjHandle] & kCanSrTxFifoIntFlag) != 0u) /* COV_CAN_GENDATA_TX_NOT_IN_ALL_CONFIG */
      {
        ...
        Can->ChFS[canPhysChannel].TRFSR[hwObjHandle] &= ((vuint32) ~kCanSrTxFifoIntFlag); /* SBSW_CAN_LL05 */ /* PRQA S 4399 */ /* MD_Can_IntegerCast */
...
\ID SBSW_CAN_LL19
 \DESCRIPTION Write access or call to CanLL_WriteRegGlobal*() for a transmit Queue related CAN cell register. The register structure and the used Tx Queue indices are static and verified by review. The channel indices are verified by a runtime check.
 \COUNTERMEASURE \S [CM_CAN_LL01]
                 \S [CM_CAN_LL02]
                 \S [CM_CAN_LL03]
                 \R [CM_CAN_LL04]
...
\CM CM_CAN_LL01 The definition of C_ENABLE_IF_RSCAN* is verified by user review [SMI-340290].
\CM CM_CAN_LL02 The value of kCanBaseAdr is verified by user review [SMI-340290].
\CM CM_CAN_LL03 The value of kCanMaxPhysChannels is verified by user review [SMI-340290].
\CM CM_CAN_LL04 A runtime check ensures that the used index is always smaller than kCanMaxPhysChannels.
\CM CM_CAN_LL05 A runtime check ensures that the used index is always smaller than kCanMaxRxFifos.
\CM CM_CAN_LL06 A runtime check ensures that the used index is always smaller than kCanMaxTxBuf.
\CM CM_CAN_LL07 A runtime check ensures that the used index is always smaller than kCanMaxTxFifos.
\CM CM_CAN_LL08 A runtime check ensures that the used index is always smaller than kCanNumberOfSharedReg16.
\CM CM_CAN_LL09 A runtime check ensures that the used index is always smaller than kCanMaxRuleAccessCell.
\CM CM_CAN_LL10 A runtime check ensures that the used index is always smaller than the data size.
\CM CM_CAN_LL11 A runtime check ensures that the data field of the CanLL_RxTmpBuf has sufficient size.
