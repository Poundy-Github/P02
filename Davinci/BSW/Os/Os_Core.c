...
))
{
  ...
  {
    if(coreIdx != currentCoreId)                                                                                        /* COV_OS_HALPLATFORMMULTICOREUNSUPPORTED */
    {
      P2CONST(Os_CoreConfigType, AUTOMATIC, OS_CONST) core;
...
)
{
  ...
  if(OS_UNLIKELY(Os_CoreCheckExceptionContextReadingSupported() == OS_CHECK_FAILED))                                    /* COV_OS_HALPLATFORMGETEXCEPTIONCONTEXTAVAILABILITY */
  {
...
)
{
  ...
  /* #10 Perform error checks */
  if(OS_UNLIKELY(Os_CoreCheckExceptionContextModificationSupported() == OS_CHECK_FAILED))                               /* COV_OS_HALPLATFORMSETEXCEPTIONCONTEXTAVAILABILITY */
  {
    status = OS_STATUS_UNIMPLEMENTED_EXCEPTIONCONTEXT;
  }
  else
...
FUNC(void, OS_CODE) Os_Api_Init(void)
{
  ...
  if(Os_CoreIsSingleCoreOs() != 0u)                                                                                     /* COV_OS_TESTSUITERESTRICTIONCORECOUNT */
  {
    Os_SystemInit();
  }
  else
