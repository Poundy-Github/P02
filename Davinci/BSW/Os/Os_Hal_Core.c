...
FUNC(void, OS_CODE) Os_Hal_CoreInit(P2CONST(Os_Hal_CoreAsrConfigType, AUTOMATIC, OS_CONST) Config)
{
  ...
  /* #30 Initialize SELB_INTCx registers if available. */
  OS_HAL_SELB_INTC1_INIT(OS_HAL_SELB_INTC1_MASK);                                                                       /* SBSW_OS_HAL_PWA_CALLER */ /* PRQA S 0303 */ /* MD_Os_Hal_Rule11.4_0303 */
  OS_HAL_SELB_INTC2_INIT(OS_HAL_SELB_INTC2_MASK);                                                                       /* SBSW_OS_HAL_PWA_CALLER */ /* PRQA S 0303 */ /* MD_Os_Hal_Rule11.4_0303 */

  /* #40 Initialize syscall table and ASID if memory protection is enabled. */
  ...
  /* #50 Restore old interrupt (EI) state. */
...
{
  /* #10 Loop endlessly. */
  while(1u == 1u)                                                                                                       /* COV_OS_HALCOREFREEZE */ /* COMP_WARN_OS_CW_005 */
  {
    Os_Hal_CoreNop();
  }
}
...
\ID COV_OS_HALPLATFORMISRMAPPINGAVAILABILITY
  \ACCEPT XX
  \ACCEPT XF
  \REASON As the interrupt mapping functionality is not supported on the RH850 platform, conditions are always false and
...
\ID COV_OS_HALPLATFORMCOREAVAILABILITY
   \ACCEPT TX
   \REASON As the platform only supports G3X and G4X cores one core specific header must always be included.
