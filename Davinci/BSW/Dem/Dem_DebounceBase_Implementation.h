  ...
  )
{
  ...
  lMaxDebounceValuePtr = (Dem_Cfg_MaxDebounceValuesPtrType)Dem_Cfg_GetMaxDebounceValuesOfSatelliteInfo(SatelliteId);             /* PRQA S 2842 */ /* MD_DEM_2842_satellite */
  ...
  )
{
  ...
  {
    Dem_Cfg_MaxDebounceValuesPtrType lMaxDebounceValuePtr;
    lMaxDebounceValuePtr = (Dem_Cfg_MaxDebounceValuesPtrType)Dem_Cfg_GetMaxDebounceValuesOfSatelliteInfo(Dem_Cfg_EventSatelliteId(EventId));
    Dem_Internal_AssertReturnValue(lMaxDebounceValuePtr != NULL_PTR, DEM_E_INCONSISTENT_STATE, lReturn)

    lReturn = (sint16)lMaxDebounceValuePtr[Dem_Cfg_EventSatelliteEventId(EventId)];
  }
  return lReturn;
}
  ...
  )
{
  ...
  lMaxDebounceValuePtr = (Dem_Cfg_MaxDebounceValuesPtrType)Dem_Cfg_GetMaxDebounceValuesOfSatelliteInfo(lSatelliteId);            /* PRQA S 2842 */ /* MD_DEM_2842_satellite */
  ...
  if (lMaxDebounceValuePtr == NULL_PTR)
  {
    ...
    /* ----- Runtime Checks ------------------------------------------------ */
  if ((Dem_Error_isDevRunTimeChecksEnabled() == TRUE)
    && ((lSatelliteId >= Dem_Cfg_GetSizeOfSatelliteInfo())                                                                       /* PRQA S 3415 */ /* MD_DEM_13.5_cf */
    || (lMonitorId >= (Dem_Cfg_GetEventCountOfSatelliteInfo(lSatelliteId)))))                                                    /* PRQA S 3415 */ /* MD_DEM_13.5_cf */
  {
    Dem_Error_RunTimeCheckFailed(DEM_DEBOUNCEBASE_IMPLEMENTATION_FILENAME, __LINE__);                                            /* SBSW_DEM_POINTER_RUNTIME_CHECK */
  }
  else
