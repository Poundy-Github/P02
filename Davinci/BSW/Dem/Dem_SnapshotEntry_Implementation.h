...
# if ((DEM_CFG_SUPPORT_OBDII == STD_ON) && (DEM_CFG_SUPPORT_OBDII_FREEZEFRAME_IN_SVC19 == STD_ON))
  ...
  DEM_IGNORE_UNUSED_CONST_ARGUMENT(CombinedDTCGroup)                                                                             /* PRQA S 3112 */ /* MD_DEM_14.2 */
  if (SRecId == DEM_DCM_SNAPSHOTDATARECORD_OBD)
  {
    lMatch = TRUE;
  }
  else
...
# else /* (DEM_CFG_SUPPORT_SRECS_CONFIGURED == STD_ON) */
  {
    for (Dem_Cfg_DTCFreezeFrameRecordIterInit(&lDTCFreezeFrameIter, CombinedDTCGroup);                                           /* SBSW_DEM_CALL_ITERATOR_POINTER */
...
{
  if ( (SnapshotEntryIter->MemoryEntryId != DEM_MEMORYENTRY_HANDLE_INVALID)
    && (EntryIndex < Dem_Cfg_GlobalSRecMaxCount()) )                                                                             /* PRQA S 3415 */ /* MD_DEM_13.5_cf */
  {
...
# endif
    ...
    Dem_Memory_MemEntry_IterNext(&lMemEntryIter))                                                                                /* SBSW_DEM_CALL_ITERATOR_POINTER */
  {
    ...
    if ((Dem_Event_TestValidHandle(lEventId) == TRUE)
      && (Dem_Cfg_EventCombinedGroup(lEventId) == GroupId))                                                                      /* PRQA S 3415 */ /* MD_DEM_13.5_cf */
    {
# if (DEM_CFG_SUPPORT_SRECS_CALCULATED == STD_ON)
