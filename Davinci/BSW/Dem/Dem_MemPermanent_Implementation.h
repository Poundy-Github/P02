    ...
    if (lMasterEventId != DEM_EVENT_INVALID)
    {
      if ((DEM_ESM_GET_PERMANENT_STATE(Dem_Mem_PermanentGetStatus(lPermanentIndex)) == DEM_ESM_PERMANENT_PASSIVE))
      {
        ...
        if ( (lWwhObdDtcClass == DEM_ESM_WWHOBD_DTCCLASS_B1)
           && (Dem_EventInternalStatus_Test_QualifyStatus_CDTC(Dem_DTC_GetQualifyStatus(lMasterEventId)) == TRUE)                /* PRQA S 3415 */ /* MD_DEM_13.5_cf */
           && (Dem_UDSStatus_Test_TF(Dem_DTC_GetDTCStatus(lMasterEventId)) == TRUE) )                                            /* PRQA S 3415 */ /* MD_DEM_13.5_cf */
        {
          uint8 lPermanentState;
