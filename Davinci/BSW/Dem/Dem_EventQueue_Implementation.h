  ...
  )
{
  ...
  return (boolean)((QueueStatus & DEM_EVENTQUEUE_SIMILAR_CONDITIONS) != 0u);
#else
  return FALSE;
#endif
}
  ...
  )
{
  ...
  if (Dem_EventQueue_TestFdcProcessingStatus(QueueStatus) == TRUE)
  {
    ...
    lTestCompleted = ( (Dem_EventQueue_GetQueueAction(QueueStatus) != 0u)
                           && (Dem_UDSStatus_Test_TNCTOC(Dem_Event_GetUDSStatus(EventId)) == TRUE) );                            /* PRQA S 3415 */ /* MD_DEM_13.5_cf */
