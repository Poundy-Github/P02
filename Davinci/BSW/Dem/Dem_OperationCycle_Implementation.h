...
#define Dem_OperationCycle_IsPendingCycle(CycleId)   ((Dem_OperationCycle_CycleFlag & ((uint16)1 << CycleId)) != 0u)             /* PRQA S 3453 */ /* MD_MSR_FctLikeMacro */
  ...
  )
{
  ...
  if ( (lOldPermanentCount == Dem_Cfg_GlobalPermanentMaxCount())
     && (lOldPermanentCount > Dem_Memory_GetNumAllocatedMemoryEntry(DEM_CFG_MEMORYID_PERMANENT)) )                               /* PRQA S 3415 */ /* MD_DEM_13.5_cf */
  ...
  )
{
  ...
  else if (Dem_Iumpr_IsIgnitionHybrid(CycleId) == TRUE)
  {
...
# endif
    {
      if ( (DEM_OPERATIONCYCLE_CYCLE_STARTED == Dem_OperationCycle_GetCurrentCycleStatus(lCycleIndex))
        && (Dem_Cfg_CycleIsAutomaticEnd(lCycleIndex) == TRUE)                                                                    /* PRQA S 3415 */ /* MD_DEM_13.5_cf */
        )
      {
# if (DEM_CFG_SUPPORT_RESTART_CYCLE == STD_ON)
