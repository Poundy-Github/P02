 ...
 *  Anna Bosch                    visanh        Vector Informatik Gmb
  ...
  )
{
  ...
  Dem_MemSet((Dem_DataPtrType)&Dem_Cfg_GetMemoryCommitNumber(0), DEM_MEMORY_INIT_COMMIT_NUMBER, Dem_Cfg_GetSizeOfMemoryCommitNumber()*sizeof(Dem_Cfg_GetMemoryCommitNumber(0))); /* PRQA S 0312, 0602 */ /* MD_DEM_11.5, MD_DEM_20.2 */ /* SBSW_DEM_MEMSET_MEMORYCOMMITNUMBER */

  lResetStoredData = (boolean)( (Dem_Cfg_AdminData.ImplementationVersion != Dem_Make16Bit(DEM_SW_MAJOR_VERSION, DEM_SW_MINOR_VERSION))
                           || (Dem_Cfg_AdminData.ConfigurationId != Dem_Cfg_GlobalConfigurationId())                             /* PRQA S 3415 */ /* MD_DEM_13.5_cf */
                           || (Dem_Memory_GetInitAdminDataRequested() == TRUE));                                                 /* PRQA S 3415 */ /* MD_DEM_13.5_cf */
  ...
  if (lResetStoredData == TRUE)
  {
    ...
    if (Dem_Memory_GetInitAdminDataRequested() == TRUE)
    {
      /* Reset flag to prevent initialization on Sleep-WakeUp */
