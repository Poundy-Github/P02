...
#define DEM_FILTERDATA_FILTEREDEVENTS_SIZE(NumEvents)         (uint16)(((NumEvents)+7)/8)                                        /* PRQA S 3453 */ /* MD_MSR_FctLikeMacro */
/*! Index of event in bitfield of already filtered events */
#define DEM_FILTERDATA_FILTEREDEVENTS_INDEX(EventId)          (uint16)((EventId)/8)                                              /* PRQA S 3453 */ /* MD_MSR_FctLikeMacro */
/*! Bitmask for bitfield of already filtered events */
#define DEM_FILTERDATA_FILTEREDEVENTS_MASK(EventId)           (uint8)(0x01u << ((uint8)((EventId) % 8)))                         /* PRQA S 3453 */ /* MD_MSR_FctLikeMacro */
...
#if ((DEM_CFG_SUPPORT_DCM == STD_ON) && (DEM_CFG_SUPPORT_PERMANENT == STD_ON))
/* ****************************************************************************
...
# if (DEM_CFG_REPORT_SUPPRESSED_DTCS_IN_19_0A == STD_ON)
      && ((Dem_FilterData_TestUseStatusMask(FilterId) == TRUE)                                                                   /* PRQA S 3415 */ /* MD_DEM_13.5_cf */
        || (Dem_FilterData_GetMemoryId(FilterId) != DEM_CFG_MEMORYID_PRIMARY))                                                   /* PRQA S 3415 */ /* MD_DEM_13.5_cf */
# endif
       ))
  {
...
# if (DEM_FEATURE_NEED_OBD == STD_ON)
  if ( (Dem_FilterData_TestUseFormatOBD(FilterId) == TRUE)
    || (Dem_FilterData_TestFilterEmissionRelevant(FilterId) == TRUE))                                                            /* PRQA S 3415 */ /* MD_DEM_13.5_cf */
  {
  ...
  { 
    ...
    if ( (Dem_DTCReporting_TestObdHideOccurrences() == TRUE)
      && (Dem_Cfg_EventSignificance(EventId) == DEM_CFG_EVENT_SIGNIFICANCE_OCCURRENCE)                                           /* PRQA S 3415 */ /* MD_DEM_13.5_cf */
  ...
  )
{
  ...
  while (lPermanentIndex != 0u)
  {
    ...
    { /* Visible Event identified, return it and stop processing */
      ...
      {
        if  ((Dem_DTC_TestEventStoresPermanentDTC(lPermanentEventId) == TRUE) && (Dem_DTC_TestSuppressionOnDtcRetrieval(lPermanentEventId) == FALSE))    /* PRQA S 3415 */ /* MD_DEM_13.5_cf */
        {
          break;
        }
        else
