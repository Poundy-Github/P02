 ...
 ** Module: E2EPW_ComEx_PEPS2_PEPS2_rx.c
 ...
 **********************************************************/
#include "ComEx.h"
#include "E2EPW_Marshal_PEPS2.h"
#include "E2EPW_CheckDeserial_PEPS2.h"
#include "E2EPW_ComEx_PEPS2_PEPS2_rx.h"
#ifndef STATIC
...
/**********************************************************
 * Function name: E2EPW_Init_PEPS2_E2E_rx ()
 ...
 **********************************************************/
FUNC (void, ComEx_CODE) E2EPW_ReadInit_PEPS2_E2E (void)
{
    State_PEPS2_E2E = &StateVal_PEPS2_E2E;
    State_PEPS2_E2E->LastValidCounter = 0;
    State_PEPS2_E2E->MaxDeltaCounter  = 0;
    State_PEPS2_E2E->WaitForFirstData = TRUE;
    State_PEPS2_E2E->NewDataAvailable = FALSE;
    State_PEPS2_E2E->LostData         = 0;
    State_PEPS2_E2E->Status           = E2E_P01STATUS_NONEWDATA;
    State_PEPS2_E2E->SyncCounter      = 0;
    State_PEPS2_E2E->NoNewOrRepeatedDataCounter = 0;
}

/**********************************************************
 * Function name: E2EPW_Get_ReceiverState_PEPS2_E2E ()
...
FUNC_P2VAR ( E2E_P01CheckStateType
    ...
    E2EPW_Get_CheckState_PEPS2_E2E (void)
{
    return State_PEPS2_E2E;
}

/**********************************************************
 * Function name: E2EPW_Read_PEPS2_E2E (ComEx_E2ETYPE * AppData);
 ...
 * Remarks:       None
 **********************************************************/
FUNC (uint32, ComEx_CODE) E2EPW_Read_PEPS2_E2E
    ( P2VAR (ComEx_E2ETYPE, AUTOMATIC, ComEx_VAR_INIT) AppData)
{
    ...
    P2CONST (E2E_P01ConfigType, AUTOMATIC, ComEx_CONST)
        Config_PEPS2_E2E = &ConfigVal_PEPS2_E2E;

    State_PEPS2_E2E->NewDataAvailable =
        Rte_IsUpdated_PEPS2_E2E ();

    if ((State_PEPS2_E2E->NewDataAvailable != FALSE))
    {
        if (AppData == NULL_PTR)
        {
        ...
        if (status_u32 == status_ok_u32)
        {
            /* Return-value of Rte_Read does not affect
               following workflow.
            */
            ret_rte_u8 = Rte_Read_PEPS2_E2E (AppData);
        }

        if (status_u32 == status_ok_u32)
        {
            ret_u8 = E2EPW_CheckDeserial_PEPS2 (AppData);
        ...
        if (status_u32 == status_ok_u32)
        {
            uint16 crcoffset_u16 = Config_PEPS2_E2E->CRCOffset >> 3u;
            uint16 cntoffset_u16 = Config_PEPS2_E2E->CounterOffset >> 3u;
            E2EPW_Marshal_PEPS2 ( AppData, ppa_PEPS2_E2E_au8 );
/**********************************************************
 ...
 **********************************************************/
/* PRQA S 505 1*/
            ppa_PEPS2_E2E_au8[crcoffset_u16]  = AppData->Checksum;
            ppa_PEPS2_E2E_au8[cntoffset_u16] &= (uint8)0xf0;
            ppa_PEPS2_E2E_au8[cntoffset_u16] |= AppData->RollingCounter;
        }
    }
    ...
    if (status_u32 == status_ok_u32)
    {
        ...
        status_u32 |= ret_u8;
        status_u32 |= (uint32)(State_PEPS2_E2E->Status<<24);
    }
